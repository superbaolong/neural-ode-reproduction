...
基础实验在 Windows 环境中完成，使用 NVIDIA GeForce RTX 3050 Laptop GPU（4GB 显存）。30-epoch 补充实验在 AutoDL 云端 NVIDIA RTX 5090（32GB 显存）实例上完成。Python 环境位于 `D:\anaconda3\envs\bishe`，主要依赖如下：
...
为进一步判断 5 个 epoch 是否足以收敛，本项目还在 RTX 5090 上使用 `seed=0` 将 ODENet 与 ResNet 各训练 30 个 epoch。除训练轮数和硬件外，其余关键超参数保持一致。该运行用于观察收敛趋势；由于只运行了一个随机种子，不与三 seed 均值混合统计。
...
### 4.3 云端 30-epoch 补充实验

在 AutoDL RTX 5090 上，使用 `seed=0` 将两个模型各训练 30 个 epoch，最终结果如下：
...
与 5-epoch 结果相比，两种模型均继续提升，ResNet 的提升最明显。这说明前述三 seed 短实验适合检查随机波动和代码可运行性，但不能代表充分训练后的 ResNet 上限。在本次单 seed 长训练中，ODENet 最终仍高出 ResNet 约 1.73 个百分点。ODENet 的平均每轮训练时间约为 ResNet 的 21.6 倍，NFE 由第一轮的 41.45 增长到第 30 轮的 82.57，反映出训练后期向量场求解成本增加。

![MNIST 云端 30 轮训练曲线](../outputs/mnist-cloud-30/training_curves.png)

完整的 60 条逐 epoch 指标、配置、运行命令和最终指标见 [`outputs/mnist-cloud-30`](../outputs/mnist-cloud-30/summary.md)。
...
首先，在参数量接近的条件下，ODENet 在三个随机种子的短实验以及 seed=0 的 30-epoch 实验中均取得高于 ResNet 基线的测试准确率，说明连续状态变换能够有效拟合该手写数字分类任务。不过，30-epoch 实验也表明 ResNet 从短实验约 92% 的水平提升到 97.54%，所以短训练结果明显低估了基线。本实验中的 ResNet 是为控制参数量而设计的轻量基线，并不是经过充分调优的标准 ResNet。因此，结果只能说明本项目实现的 ODENet 优于本项目实现的轻量残差基线，不能推广为 Neural ODE 普遍优于所有 CNN 或 ResNet。
...
4. ResNet 基线没有进行学习率调度或数据增强；
5. 实验分别在 RTX 3050 Laptop 和 RTX 5090 上运行，跨硬件时间数据不可直接比较；
6. 30-epoch 实验只运行了 `seed=0`，三随机种子统计仍来自 5-epoch 短实验；
7. 三个随机种子满足课程复现报告的基本统计要求，但仍不足以支持广泛的模型优劣结论。

## 7. 结论

本项目完成了 Neural ODE 核心思想的可运行复现。Spiral 实验验证了神经网络向量场学习连续动力系统的能力；MNIST 的三 seed、5-epoch 实验中，ODENet 与 ResNet 的最佳测试准确率分别为 `98.00% ± 0.40%` 和 `92.35% ± 1.21%`。在 seed=0 的 30-epoch 补充实验中，两者最终达到 `99.27%` 和 `97.54%`，说明短训练明显低估了 ResNet，同时 ODENet 在该轻量、参数量匹配的设置下仍保持优势。与此同时，ODENet 需要显著更多的训练时间和显存。伴随灵敏度实验进一步说明，通过反向求解增广 ODE 可以降低显存，但会增加 NFE 和运行时间。总体而言，Neural ODE 提供了一种将网络深度连续化、并借助数值分析工具控制计算的建模方式，但其工程价值需要结合准确率、求解稳定性和计算成本综合评估。
